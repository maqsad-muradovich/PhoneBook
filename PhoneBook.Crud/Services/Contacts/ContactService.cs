...
namespace PhoneBook.Crud.Services.Contacts
{
    internal class ContactService : IContactService
    {
        ...
        {
            Contact[] oldContacts = this.storageBroker.ReadAllContacts();
            Contact[] newContacts = this.storageBroker.DeleteContact();

            int oldContactsLength = oldContacts.Length;
            int newContactsLength = newContacts.Length;

            if(oldContactsLength == newContactsLength)
            {
                this.loggingBroker.LogError("Contact ochirilmadi");
            }
            else if(oldContactsLength > newContactsLength)
            {
                this.loggingBroker.LogError("Contact Ochiridi");
            }
